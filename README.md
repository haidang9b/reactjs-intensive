...
[![Tests](https://img.shields.io/badge/tests-78%20passing-brightgreen)](https://github.com/haidang9b/reactjs-intensive/actions/workflows/deploy.yml)
[![Coverage](https://img.shields.io/badge/coverage-81%25-brightgreen)](https://github.com/haidang9b/reactjs-intensive/actions/workflows/deploy.yml)
...
The `Deploy to GitHub Pages` pipeline runs `lint → typecheck → test → coverage → build`
on every push to `main` (coverage is printed to the job summary). Latest results:

| Check       | Result                                                              |
| ----------- | ------------------------------------------------------------------ |
| Test result | **78 passed** / 78 total (36 test files)                           |
| &nbsp;      | `@react-workshop/web` — 74 passed (34 files)                       |
| &nbsp;      | `@react-workshop/ui` — 2 passed · `@react-workshop/http-client` — 2 passed |
| Build       | **passing** — 2/2 tasks (`turbo build`)                            |
| Build time  | ~1.3s cold (web app via Vite); ~0.02s cached (Turbo full-cache hit) |

### Code coverage (`apps/web`)

Generated by `pnpm --filter @react-workshop/web test:coverage` (Vitest v8 provider;
app wiring — router/providers/entry — excluded):
...
> All four metrics exceed the workshop's **70%** passing criterion.
